...
## 11. Meta Pixel & Conversion Tracking
- **Pixel** `1639147300509579` base code + `PageView` injected into `<head>` of all 74 pages.
- **Custom events** (Option A — fire `fbq` events only, no redirects, no layout impact), wired via a small delegated script before `</body>`:
  - `ViewContent` — curriculum/course pages (uk/us/uae sub-pages + class/subject/course pages).
  - `Lead` — lead popup submit + enquiry / "Book a Free Demo" (`ed-contact__form-main`) form submits.
  - `CompleteRegistration` — "Submit Trial" (final step of the Book Your Free Trial modal).
  - `Contact` — WhatsApp link clicks (delegated `a[href*="wa.me"]`).
- All events verified firing in-browser (`facebook.com/tr?ev=…` → 200).
